...
<  RENDER: generateScript >
  ...
  2. **Webinar Title & Description**  
     ...
     - Then ask for **following details** :
  ...
  3.  **Confirm Detials**  
     - Once Details provided by user, Ask for any changes/confirmation regarding the Webinar Details. (Do not use "TRIGGER: generateKnowledgeBase" during confirmation) 
...
// console.log('Webinar Details:', webinarDetails);
  ...
  const generateKnowledgeBaseWithAI = async () => {
    ...
    toast.warn('Generating KnowledgeBase with AI');
    ...
    <ToastContainer/>
      {!isOpen && (
        <button
          ...
          data-tour="chatbot"
          className="fixed bottom-4 right-4 bg-teal-400 text-black rounded-full p-3 shadow-lg hover:bg-teal-600 transition-colors"
        >
